...
#include "../headers/lv2_uri_map.h"
#include "../headers/lv2_saverestore.h"
#include "../headers/lv2_event.h"
#include "../headers/lv2_contexts.h"
#include "../headers/lv2types.hpp"
...
namespace LV2
{
	...
	struct FixedBufSize
	{
			...
			template<class Derived> struct I: Extension<Required>
			{
					...
					{
						hmap["http://tapas.affenbande.org/lv2/ext/fixed-buffersize"] = &I<Derived>::handle_feature;
					}
	...
	struct FixedP2BufSize
	{
			...
			template<class Derived> struct I: Extension<Required>
			{
					...
					static void map_feature_handlers(FeatureHandlerMap& hmap)
					{
						hmap["http://tapas.affenbande.org/lv2/ext/power-of-two-buffersize"] = &I<Derived>::handle_feature;
					}
	...
	struct SaveRestore
	{
			...
			template<class Derived> struct I: Extension<Required>
			{
					...
					static const void* extension_data(const char* uri)
					{
						...
						{
							static LV2SR_Descriptor srdesc =
							{ &I<Derived>::_save, &I<Derived>::_restore };
							return &srdesc;
						}
						return 0;
					}
				...
				protected:
					...
					static char* _save(LV2_Handle h, const char* directory, LV2SR_File*** files)
					{
	...
	struct EventRef
	{
			...
			template<class Derived> struct I: Extension<Required>
			{
					...
					/** @internal */
					I() :
							m_callback_data(0), m_ref_func(0), m_unref_func(0)
					{
					}
	...
	struct MsgContext
	{
			...
			template<class Derived> struct I: Extension<Required>
			{
					...
					static const void* extension_data(const char* uri)
					{
						...
						{
							static LV2_Blocking_Context desc =
							{ &I<Derived>::_blocking_run, &I<Derived>::_connect_port };
							return &desc;
						}
						return 0;
					}
