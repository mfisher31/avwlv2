...
namespace LV2
{
	...
	struct NoUserResize
	{
			...
			template<class Derived> struct I: public Extension<Required>
			{
					...
					{
						hmap["http://ll-plugins.nongnu.org/lv2/dev/ui#noUserResize"] = &I<Derived>::handle_feature;
					}
	...
	struct FixedSize
	{
			...
			template<class Derived> struct I: public Extension<Required>
			{
					...
					static void map_feature_handlers(FeatureHandlerMap& hmap)
					{
						hmap["http://ll-plugins.nongnu.org/lv2/dev/ui#fixedSize"] = &I<Derived>::handle_feature;
					}
	...
	struct WriteMIDI
	{
			...
			template<class Derived> struct I: Extension<Required>
			{
					...
					I() :
							m_midi_type(0)
					{
						m_buffer = lv2_event_buffer_new(sizeof(LV2_Event) + EVENT_BUFFER_SIZE, 0);
					}
					...
					bool check_ok()
					{
						...
						m_midi_type = d->uri_to_id(LV2_EVENT_URI, "http://lv2plug.in/ns/ext/midi#MidiEvent");
						m_event_buffer_format = d->uri_to_id(LV2_UI_URI, "http://lv2plug.in/ns/extensions/ui#Events");
				...
				protected:
					...
					bool write_midi(uint32_t port, uint32_t size, const uint8_t* data)
					{
						...
						static_cast<Derived*>(this)->write(port, m_buffer->header_size + m_buffer->capacity, m_event_buffer_format, m_buffer);
						if (size > 4)
							std::free(buffer);
						return true;
					}
