...
void VcaExpCV::run(uint32_t nframes)
{
	...
	for (unsigned int l2 = 0; l2 < nframes; l2++)
	{
		...
		p(p_out)[l2] = g * *p(p_outputLevel) * (*p(p_in1) * p(p_in1Data)[l2] + *p(p_in1) * p(p_in2Data)[l2]);
	}
}

static int _ = VcaExpCV::register_class("http://avwlv2.sourceforge.net/plugins/avw/vcaexp_cv");
