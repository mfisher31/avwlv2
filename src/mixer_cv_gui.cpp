...
#include "mixer_cv_gui.hpp"
#include "mixer_cv.hpp"

MixerCVGUI::MixerCVGUI(const std::string& URI)
{
	...
	slot<void> p_slotGain = compose(bind<0>(mem_fun(*this, &MixerCVGUI::write_control), p_gain), mem_fun(*this,  &MixerCVGUI::get_gain));
	...
	slot<void> p_slotVolume1 = compose(bind<0>(mem_fun(*this, &MixerCVGUI::write_control), p_volume1), mem_fun(*this,  &MixerCVGUI::get_volume1));
	m_dialVolume1 = new LabeledDial("Volume 1", p_slotVolume1, p_volume1, 0, 2, false, 0.01, 2);
	p_volumesBox->attach(*m_dialVolume1, 0, 1, 0, 1);

	slot<void> p_slotVolume2 = compose(bind<0>(mem_fun(*this, &MixerCVGUI::write_control), p_volume2), mem_fun(*this,  &MixerCVGUI::get_volume2));
	m_dialVolume2 = new LabeledDial("Volume 2", p_slotVolume2, p_volume2, 0, 2, false, 0.01, 2);
	p_volumesBox->attach(*m_dialVolume2, 1, 2, 0, 1);

	slot<void> p_slotVolume3 = compose(bind<0>(mem_fun(*this, &MixerCVGUI::write_control), p_volume3), mem_fun(*this,  &MixerCVGUI::get_volume3));
	m_dialVolume3 = new LabeledDial("Volume 3", p_slotVolume3, p_volume3, 0, 2, false, 0.01, 2);
	p_volumesBox->attach(*m_dialVolume3, 0, 1, 1, 2);

	slot<void> p_slotVolume4 = compose(bind<0>(mem_fun(*this, &MixerCVGUI::write_control), p_volume4), mem_fun(*this,  &MixerCVGUI::get_volume4));
...
float MixerCVGUI::get_gain(){ return m_dialGain->get_value(); }
float MixerCVGUI::get_volume1(){ return m_dialVolume1->get_value(); }
float MixerCVGUI::get_volume2(){ return m_dialVolume2->get_value(); }
float MixerCVGUI::get_volume3(){ return m_dialVolume3->get_value(); }
float MixerCVGUI::get_volume4(){ return m_dialVolume4->get_value(); }


void MixerCVGUI::port_event(uint32_t port, uint32_t buffer_size, uint32_t format, const void* buffer)
{
	if (port == p_gain)
	{
...
static int _ = MixerCVGUI::register_class("http://avwlv2.sourceforge.net/plugins/avw/mixer_cv/gui");
