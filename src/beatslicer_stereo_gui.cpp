...
BeatSlicerStereoGUI::BeatSlicerStereoGUI(const std::string& URI)
{
	...
	slot<void> p_slotTempo = compose(bind<0>(mem_fun(*this, &BeatSlicerStereoGUI::write_control), p_tempo), mem_fun(*this,  &BeatSlicerStereoGUI::get_tempo));
	m_dialTempo = new LabeledDial("Tempo", p_slotTempo, p_tempo, 40, 320, NORMAL, 1, 0);
	p_beatDials->pack_start(*m_dialTempo);

	slot<void> p_slotBeatSize = compose(bind<0>(mem_fun(*this, &BeatSlicerStereoGUI::write_control), p_sliceSize), mem_fun(*this,  &BeatSlicerStereoGUI::get_sliceSize));
	...
	slot<void> p_slotSampleSize = compose(bind<0>(mem_fun(*this, &BeatSlicerStereoGUI::write_control), p_sampleSize), mem_fun(*this,  &BeatSlicerStereoGUI::get_sampleSize));
	...
	slot<void> p_sloReverseMode = compose(bind<0> (mem_fun(*this, &BeatSlicerStereoGUI::write_control), p_reverseMode), mem_fun(*m_comboReverseMode, &ComboBoxText::get_active_row_number));
	...
	slot<void> p_slotAttack = compose(bind<0>(mem_fun(*this, &BeatSlicerStereoGUI::write_control), p_attack), mem_fun(*this,  &BeatSlicerStereoGUI::get_attack));
	m_dialAttack = new LabeledDial("Attack", p_slotAttack, p_attack, 3, 25, NORMAL, 1, 0);
	p_envelopeDials->pack_start(*m_dialAttack);

	slot<void> p_slotRelease = compose(bind<0>(mem_fun(*this, &BeatSlicerStereoGUI::write_control), p_release), mem_fun(*this,  &BeatSlicerStereoGUI::get_release));
...
float BeatSlicerStereoGUI::get_tempo() 			{ return m_dialTempo->get_value(); }
float BeatSlicerStereoGUI::get_sliceSize() 		{ return m_dialSliceSize->get_value(); }
float BeatSlicerStereoGUI::get_sampleSize() 	{ return m_dialSampleSize->get_value(); }
float BeatSlicerStereoGUI::get_attack() 		{ return m_dialAttack->get_value(); }
float BeatSlicerStereoGUI::get_release() 		{ return m_dialRelease->get_value(); }

void BeatSlicerStereoGUI::port_event(uint32_t port, uint32_t buffer_size, uint32_t format, const void* buffer)
{
...
static int _ = BeatSlicerStereoGUI::register_class("http://avwlv2.sourceforge.net/plugins/avw/beatslicer_stereo/gui");
